...
The *between* operator allows us to specify a range of values to filter by.
...
If you want to force certain columns to the front of the dataframe you can use the *everything()* function within the select function.
...
As you can see we now have a mean for each group. When aggregating it is recommended that you provide a count (**n()** gives a count of the values in a group) or a count of the non NA values **sum(!is.na(x))**. This will prevent you having a false understandings of what is happening behind the scences.
