...
The plot roughly shows a *negative relationship* between fuel efficiency and a cars engine size. There as engine size increases, the fuel economy decreases.
...
Again this type of graph enables us to gain a reasonable of information from the data points.
...
As you can see the the shape aesthetics is applied to the geom_point layer only but it is still coloured according to the global attribute. You can also take advantage of this attribute to get the geom layes to display different data.
...
As you can see emmitting the geom does not matter in this scenario. **Every stat has a default geom and every geom has a default stat**. In this case the default geom form stat_count is the bar geom, therefore a bar chart is displayed.
...
In this case we need the group attribute to be equal to 1. The reason for this is, by default geom bar will group by the values on the x axis and provide a count of each seperatly. But, as we are looking the proportions we have to look at the proportion of each in relation to the total of all the cuts. A better explanation of this can be found here *https://stackoverflow.com/questions/39878813/r-ggplot-geom-bar-meaning-of-aesgroup-1*.
...
*Identity*: essentially the same as a stacked bar chart
...
*coord_flip()*: switches the x an y axis. Useful if you want to have horizontal box plots
